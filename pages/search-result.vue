<template>
  <breadcrumb title="Home" subtitle="Search Result" alignment="start" />
...
const { categoryProducts, allProductsCat, searchResult } =
  storeToRefs(productStore);
const { getProductsOfCategory, getAllProductsCat, searchProducts } =
  productStore;
...
//watch qr query and in the result page if there new seach make it and show new result
watch(
  () => route.query,
  async (newValue, oldValue) => {
    await searchProducts(newValue.qr);
  }
);

//manage product store select box
const sortInput = reactive({
  selectedValue: "Default Sorting",
  isVisible: false,
  sortCriterias: ["Low To Hight", "Hight To Low", "New Added", "On Sale"],
});
